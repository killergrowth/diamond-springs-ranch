...
const DSR_FOLDER = '1xhBVFRLQ9Y7deUSSU2ZPasM3AfUZKd5n';

getToken(token => {
  // List all files in DSR folder (recursive by listing folder contents)
  const q = encodeURIComponent(`'${DSR_FOLDER}' in parents and trashed=false`);
  driveGet(token, `/drive/v3/files?q=${q}&fields=files(id,name,mimeType,parents)&pageSize=100`, data => {
    console.log('=== DSR CLIENT PHOTOS FOLDER CONTENTS ===');
