    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        <tr><td style="background:${primaryColor};padding:36px 40px;text-align:center;border-radius:8px 8px 0 0;">
          ...
          <div style="color:rgba(255,255,255,0.8);font-size:13px;font-weight:600;letter-spacing:3px;text-transform:uppercase;margin-top:8px;">New Estimate Request</div>
        </td></tr>
        <tr><td style="background:#fff;padding:36px 40px;">
          <p style="margin:0 0 24px;color:#333;font-size:15px;line-height:1.6;">A new estimate request was submitted through the ${clientName} website.</p>
...
// ⚠ Replace these with client values when scaffolding a new site
const CLIENT_NAME      = '{{CLIENT_NAME}}';
const CLIENT_ADDRESS   = '{{CLIENT_ADDRESS}}';
const CLIENT_PHONE     = '{{CLIENT_PHONE}}';
const PRIMARY_COLOR    = '{{CLIENT_PRIMARY_COLOR}}';
const SECONDARY_COLOR  = '{{CLIENT_SECONDARY_COLOR}}';
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const subject  = `New Estimate Request - ${CLIENT_NAME}${service ? ' (' + service + ')' : ''}`;
